...
```{r, explanatory-variables-render, echo = FALSE, cache = FALSE, message = FALSE}
...
In the plot there are some gaps between the points because, in order to reduce the execution time, the algorithm computed has been a random grid search, that means that just a random subset of grid has been tested. The time constraint of `r str_extract(results$time_string[2], ".*m")` was an input of the algorithm.
...
In table \@ref(tab:models-results) also the execution time is reported. For the models based on AIC the reported time is given by the sum of the execution time of the stepwise algorithm (`r results %>% filter(model == "glm aic") %>% .$time1_string %>% str_extract(".*m")`) and the execution time of the further fitting. Comparing the models by the execution time, we see that running an Elastic Net on the whole dataset is much more efficient than running a stepwise algorithm based on AIC. This difference in efficiency would get even bigger by increasing the number of variables in the model, as the complexity of the stepwise algorithm grows exponentially with the number of explanatory variables.
