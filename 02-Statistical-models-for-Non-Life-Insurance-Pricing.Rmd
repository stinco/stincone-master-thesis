...
In this section we will start by describing the \ac{glm}, that is the most employed model in technical pricing, to then present some of its advancements: the \ac{gam}, the Shrinkage estimators for \ac{glm} and the Bayesian \ac{glm}.
...
<!-- The design matrix is represented in figure \@ref(fig:design-matrix). --> 
...
Another method for variable selection is comparing the models by computing _information criteria_. Two information criteria commonly used are the \ac{aic} and the \ac{bic}:
\begin{align*}
