...
The common case is that $n_i^{(t)} < n_i$ and $z_j^{(t)} < z_j$. If we used $n_i^{(t)}$ and $z_j^{(t)}$ without any correcton, we would underestimate both $E(N)$ and $E(Z)$, obtaining a distorted estimate for $E(S)$.
...
- Momento di valutazione: non totalmente liquidato, non denunciato
- Outstanding Loss Liabilities: la passività emerge in t1
- Consolidamento (consolidation) dei sinistri
- Variabile risposta: Pagato + Riservato (we trust reserving)
  + IBNYR
  + IBNER
- Correttivi, proiezioni a costo ultimo. Valutazioni aggregate. Triangoli di runoff. Chain Ladder. Possono essere fatti gestione per gestione. Es: sinistri con lesioni hanno solitamente tempi di consolidamento più lunghi. Potrebbero esserci contenziosi.
- Le OLL devono essere stimate a livello aggregato anche per valutare la posizione debitoria dell'azienda e vanno riportate a bilancio.
-->
...
## Commercial Tariff and Offer Price

In section \@ref(chap:risk-prem-tech-price) we defined the _Risk Premium_ $P^{(risk)} = E(S)$ and the _Technical Price_ $P^{(tech)} = E(S) + E$ and in section \@ref(chap:personalization) we described how the risk premium is estimated. In this thesis we are not going to deal with the estimate of the expenses. In this section we are going to discuss what the tariff and the offer price are and which are the further needs that the offer should satisfy.
