...
```{r, plot-poisson, echo = FALSE, fig.cap = "Poisson distribution for some values of $\\lambda$.", fig.align = "center", fig.width = 6, fig.height = 3, cache = TRUE}
tibble(x = 0:22) %>% 
  crossing(
    tibble(
      lambda = c(.8, 1, 2.5, 10)
...
```{r, plot-gamma, echo = FALSE, fig.cap = "Gamma distribution for some values of $\\alpha$ and $\\rho$.", fig.align = "center", fig.width = 6, fig.height = 3, cache = TRUE}
...
Another characteristic of Gamma distribution that could be problematic in modeling claims severity is that it has a light tail. This means that, as $z$ goes to $+\infty$, $f_Z(z)$ aproaches $0$ quite fast. This could lead to a poor fitting for _large claims_. Other distributions with havier tails are for example the _log-Normal_ and the _Pareto_.
...
```{r, plot-binomial, echo = FALSE, fig.cap = "Binomial distribution for some values of $n$ and $p$.", fig.align = "center", fig.width = 6, fig.height = 3, cache = TRUE}
