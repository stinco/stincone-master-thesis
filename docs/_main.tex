...
In this section I am going to better explin how pricing based on policyholder information works.

Usually for every policyholder we have a set of pices of information on him that are considered relevant for his risk evaluation. This information must be reliable and observable at the moment of the underwriting of the policy.

In motor insurances these pices of information could be:
...
\item
  Policy options: amount of the maximum coverage, presence and amount of the deductible, presence of other insurance guarantees.
\end{itemize}

Formally this information can be represented as a set of variables \(\boldsymbol{x}_i=(x_{i1}, x_{i2}, \dots, x_{ip})\). These variables could be called pricing variables, explanatory variables, covariates, predictors or features.
...
The process of pricing consist in defining a Pricing Rule based on observed data from the past and assumptions on the future.

\hypertarget{pricing-variables-encoding}{%
\subsubsection{Pricing variables encoding}\label{pricing-variables-encoding}}
...
If a variable \(x\) has only 2 possible modality, it can easily encoded in a binary variable \(z\) that assigns \(0\) to one modality and \(1\) to the other. For example, if \(x = \text{sex}\), it can be encoded this way:
\[
z = \begin{cases}
...
In general, if a variable \(x\) has \(K\) modality, it can be encoded in \(K-1\) binary variables \(z_1, z_2, \dots, z_{K-1}\). For example, if \(x = \text{make}\), and it could have 4 possible modality (`Fiat', `Alfa-Romeo', `Lancia', `Ferrari') it can be encoded this way:
...
\caption{\label{tab:dummy-variables}Dummy variable encoding.}
\centering
...
\hypertarget{response-variables-and-models}{%
\subsubsection{Response variables and models}\label{response-variables-and-models}}

As we observed in section \ref{tcc-dist}, under the compound distribution hypotesis, it is not needed to model directly the total cost of claims \(S\), but we can separately model \(N\) and \(Z\).

Modeling the claims count \(N\) and the claims severity \(Z\) means find respectively:

\begin{itemize}
\tightlist
\item
  find a function \(r_N:\mathcal{X}\rightarrow R_+\) that given a set of explanatory variables \(\boldsymbol{x}_i\) will return the expected claims count for that risk \(E(N_i)\);
\item
  find a function \(r_Z:\mathcal{X}\rightarrow R_+\) that given a set of explanatory variables \(\boldsymbol{x}_i\) will return the expected severty for that risk \(E(Z_i)\).
\end{itemize}

In this context the variables \(N\) and \(Z\) are called \emph{response varables}.
