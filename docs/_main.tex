...
\textbf{Make} & \textbf{$z_1$} & \textbf{$z_2$} & \textbf{$z_3$}\\
...
\toprule\addlinespace
Normal & \makecell[c]{$N(\mu, \sigma^2)$,\\$\mu\in\mathbb{R}, \ \sigma>0$} & $\mathbb{R}$ & $\mu$ & $]0, +\infty[$ & $\sigma^2$ & $\frac{\theta^2}{2}$\\
\addlinespace\hline\addlinespace
Poisson & \makecell[c]{$Poisson(\mu)$,\\$\mu>0$} & $\mathbb{R}$ & $\log{(\mu)}$ & $\left\{1\right\}$ & $1$ & $e^{\theta}$\\
\addlinespace\hline\addlinespace
Gamma & \makecell[c]{$Gamma(\alpha, \mu)$,\\$\alpha>0, \ \mu>0$} & $]-\infty, 0[$ & $-\frac{1}{\mu}$ & $]0,+\infty[$ & $\frac{1}{\alpha}$ & $-\log{\left(-\theta\right)}$\\
\addlinespace\hline\addlinespace
...
From \eqref{eq:exp-fam-expected-value} we we obtains that, as \(\mu = b'(\theta)\), choosing the canonical function corresponds in using \(\theta\) as the linear predictor:
\[
...
In the Gamma case, its canonical function \(g(\mu)=-\frac{1}{\mu}\) has the drowback that it links the expected values \(\mu\in]0,+\infty[\) to \(\eta\in]-\infty, 0[\). This requires some constraints on \(\boldsymbol{\beta}\) because \(\eta=\boldsymbol{x}^t\boldsymbol{\beta}\) must be \(>0\). For this reason, it is preferred to use \(g(\mu) = log(\mu)\) that maps \(]0, +\infty[\) to \(\mathbb{R}\).

\hypertarget{model-fitting}{%
\subsubsection{Model fitting}\label{model-fitting}}

The model depends on the parameters \(\boldsymbol{\beta}, \phi\). Indeed, the parameters \(\theta_i\) can be obtained by \(\boldsymbol{\beta}\) as:

\[
\theta_i = b'^{-1}(\mu_i) = b'^{-1}(g^{-1}(\eta_i)) = b'^{-1}\left(g^{-1}\left(\boldsymbol{x}_i^t\boldsymbol{\beta}\right)\right)
\]

Therefore, fitting the model corresponds to estimate \(\boldsymbol{\beta}, \phi\). The technique used in GLM to estimate \(\boldsymbol{\beta}, \phi\) is the \emph{maximum likelihood}.

Let's indicate with \(l(\boldsymbol{\beta}, \phi, \boldsymbol{y})\) the model likelihood.
