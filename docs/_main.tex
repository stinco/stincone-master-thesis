...
With the parametrization of this GLM we fitted an Elastic Net. To choose the optimal parameters \(\alpha\) and \(\lambda\) we computed an grid search with a cross validation. In the cross validation we tested 363 models with a total computation time of 1h 30m.

In figure \ref{fig:en-tot-hp-tuning-plot} the results of the cross validation are shown.
...
\caption[Elastic Net Tot hyper-parameter tuning.]{Elastic Net Tot hyper-parameter tuning.}\label{fig:en-tot-hp-tuning-plot}
\end{figure}
