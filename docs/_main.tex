...
The dataset used is an actuarial dataset from an Italian MTPL portfolio. In table \ref{tab:years-at-risk} the total exposure of the dataset is reported. In the dataset there are \ensuremath{2.83829\times 10^{5}} rows for a total of \ensuremath{1.348047\times 10^{5}} years-at-risk. In the dataset, every row is a couple (policy, accounting year), so, if a policy spans in two years, we will see two rows corresponding to that policy. On average, every row has exposure 0.475 years-at-risk. This number is slightly lower than 0.5 because some of the policies get suspended and span in more than two years.

\begin{table}[!h]

\caption{\label{tab:years-at-risk}Total dataset exposure.}
\centering
\begin{tabular}[t]{ccc}
\toprule
\textbf{Observations} & \textbf{Years at Risk} & \textbf{Average Exposure}\\
\toprule\addlinespace
283829 & 134804.7 & 0.475\\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\label{tab:exposure-train-test}Training set and test set description.}
\centering
\begin{tabular}[t]{lcccc}
\toprule
\textbf{Set} & \textbf{Observations} & \textbf{Y.a.R.} & \textbf{Policyholders} & \textbf{\makecell[c]{Y.a.R. per\\Policyholder}}\\
\toprule\addlinespace
Train & 227226 & 107998.4 & 27346 & 3.95\\
\addlinespace\hline\addlinespace
Test & 56603 & 26806.3 & 6824 & 3.93\\
\bottomrule
\end{tabular}
\end{table}
