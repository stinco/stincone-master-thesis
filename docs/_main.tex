...
Usually for every policyholder we have a set of pieces of information on him that are considered relevant for his risk evaluation. This information must be reliable and observable at the moment of the underwriting of the policy.
...
\tightlist
\item
  find a function \(r_N:\mathcal{X}\rightarrow R_+\) that, given a set of explanatory variables \(\boldsymbol{x}_i\), will return the expected claims count for that risk \(E(N_i)\) and eventually other moments of \(N_i\) or even the full distribution of \(N_i\);
\item
  find a function \(r_Z:\mathcal{X}\rightarrow R_+\) that, given a set of explanatory variables \(\boldsymbol{x}_i\), will return the expected severity for that risk \(E(Z_i)\) and eventually other moments of \(N_i\) or even the full distribution of \(Z_i\).
\end{itemize}

In this context the variables \(N\) and \(Z\) are called \emph{response variables}.
