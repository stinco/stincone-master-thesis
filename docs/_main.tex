...
The higher \(K\) is, the less subjected to randomness \(D^{CV}\) is. However, the higher \(K\) is, the more computationally heavy the procedure is. A common choice for \(K\) is \(K=10\). If we choose \(K=n\), the procedure is also called \emph{leave-one-out cross validation}.
