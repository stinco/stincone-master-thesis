...
\caption{\label{tab:simulation-random}Distribution of the p-values from the simulation of the random variables. With a random variable, these frequencies should be around 0.05. With the random by policyholder, the frequencies for low p-values are higher than that.}
\centering
\begin{tabular}[t]{lcccc}
\toprule
...
In the dataset we have a total of 131 explanatory variables that are considered meaningful for predicting the claims frequency. In table \ref{tab:explanatory-variables} we can see the number of variables split by category. The categories are the ones described in section \ref{chap:pricing-variables}.
...
\begin{tabular}[t]{>{}lc}
\toprule
\em{\textbf{Description}} & \textbf{\makecell[c]{Number of variables\\per category}}\\
\toprule\addlinespace
\em{Information on the insured vehicle} & 18\\
\addlinespace\hline\addlinespace
\em{General information of the policyholder} & 36\\
\addlinespace\hline\addlinespace
\em{Insurance specific information of the policyholder} & 52\\
\addlinespace\hline\addlinespace
\em{Policy options} & 13\\
\addlinespace\hline\addlinespace
\em{Customer information on the policyholder} & 5\\
\addlinespace\hline\addlinespace
\em{Telematic data} & 7\\
\addlinespace\hline\addlinespace
\em{\textbf{Total}} & \textbf{131}\\
\bottomrule
\end{tabular}
\end{table}
