...
\hypertarget{tcc-dist}{%
\subsection{Distribution of the total cost of claims}\label{tcc-dist}}
...
First of all the insurer has to cover also the expenses related to the policy (commission on sales and expenses related to the claim settlement) and the general expenses of the company. Adding the expenses we obtain what is called \emph{Technical Price}.
...
These pieces of informations are usually called \emph{pricing variables}.
...
\tightlist
\item
  \emph{Quantitative variables}: variables like vehicle mass or policyholder age that can be easily represented as a number;
\item
  \emph{Qualitative variables}: variables like sex or address that represent a category and are usually represented with strings.
\end{enumerate}
...
\label{def:pricing-rule} \iffalse (Pricing Rule) \fi{} A \emph{Pricing Rule} is a function \(f(\cdot)\) that from an istance of a set of pricing variables \(\boldsymbol{x}_i\in\mathcal{X}\) returns a price:
