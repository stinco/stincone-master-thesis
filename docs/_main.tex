...
\hypertarget{some-considerations-on-shrinkage-estimators}{%
\subsubsection{Some considerations on Shrinkage Estimators}\label{some-considerations-on-shrinkage-estimators}}
...
\caption[Prior \(\pi(\mu)\), likelihood \(\pi(\mu|\boldsymbol{y})\) and posterior distribution \(\pi(\mu|\boldsymbol{y})\) for estimate of the mean from a Normal distribution.]{Prior \(\pi(\mu)\), likelihood \(\pi(\mu|\boldsymbol{y})\) and posterior distribution \(\pi(\mu|\boldsymbol{y})\) for estimate of the mean from a Normal distribution. The panels shows \(\pi(\mu)\), \(\pi(\mu|\boldsymbol{y})\) and \(\pi(\mu|\boldsymbol{y})\) for different values of the prior variance \(\sigma_0\) (rows) of \(\mu\) and different sample size \(n\) (columns).}\label{fig:bayes-normal}
\end{figure}
...
If in equation \eqref{eq:bayes-max-lik-est} we substitute \(L\left(\boldsymbol{\beta}, \phi \mid \boldsymbol{y}\right)\) with \(\pi\left(\boldsymbol{\beta}, \phi \mid \boldsymbol{y}\right)\), we obtain the Maximum a posteriori Estimator \(\tilde{\boldsymbol{\beta}}^{ML}\)
\begin{equation}
...
\hypertarget{chap:ridge-lasso-bayes}{%
\subsubsection{LASSO and Ridge Regression as Bayesian estimators}\label{chap:ridge-lasso-bayes}}
...
\hypertarget{some-considerations-on-bayesian-estimators}{%
\subsubsection{Some considerations on Bayesian estimators}\label{some-considerations-on-bayesian-estimators}}
