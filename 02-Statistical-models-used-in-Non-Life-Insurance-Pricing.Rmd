...
$$
At this point we will choose the hyper-parameter that minimizes the cross-validation deviance:
$$
...
$$
```
One consequence of this thorem is that, within all the continuous function $f(\cdot)$ with continuous first and second derivatives, the one that minimize the optimization problem \@ref(eq:gam-est-deviance) is always a natural cubic splines.
...
In section \@ref(chap:cubic-splines) we said that natural cubic splines on knots $\nu_1, \dots, \nu_m$ contitute a $m$-dimensional vectorial space and that a possible basis decomposition is \@ref(eq:cubic-spline-decomposition).

Another usual basis is th _B-spline_ basis. B-splines are functions defined recursively as:
