...
```{r, plot-cub-spline-print, out.width = "50%", fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:cub-spline-caption-gitbook)", "(ref:cub-spline-caption-latex)"), fig.scap="(ref:cub-spline-caption-short)", label="cub-spline", echo=FALSE, fig.ncol=2, fig.subcap=c('$0$ knots', '$3$ knots', '$7$ knots', '$35$ knots'), cache = TRUE}
...
```{r, plot-gam-lambda-print, out.width = "50%", fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:gam-lambda-caption-gitbook)", "(ref:gam-lambda-caption-latex)"), fig.scap="(ref:gam-lambda-caption-short)", label="gam-lambda", echo=FALSE, fig.ncol=2, fig.subcap=c('$\\lambda = 0$', '$\\lambda = 10$', '$\\lambda = 10^3$', '$\\lambda = 10^6$'), cache = TRUE}
...
```{r, plot-ridge-lambda-print, out.width = c("50%", "50%", "50%", "50%", "70%"), fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:ridge-lambda-caption-gitbook)", "(ref:ridge-lambda-caption-latex)"), fig.scap="(ref:ridge-lambda-caption-short)", label="ridge-lambda", echo=FALSE, fig.ncol=2, fig.subcap = c(str_c('$\\lambda = ', lambda_ridge, '$'), '$\\hat{\\beta}_j$ for different values of $\\lambda$'), cache = TRUE}
...
```{r, plot-lasso-lambda-print, out.width = c("50%", "50%", "50%", "50%", "70%"), fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:lasso-lambda-caption-gitbook)", "(ref:lasso-lambda-caption-latex)"), fig.scap="(ref:lasso-lambda-caption-short)", label="lasso-lambda", echo=FALSE, fig.ncol=2, fig.subcap = c(str_c('$\\lambda = ', lambda_lasso, '$'), '$\\hat{\\beta}_j$ for different values of $\\lambda$'), cache = TRUE}
...
(ref:opt-ridge-lasso-caption-gitbook) Geometrical interpretation of the optimization problem for Ridge and LASSO, Ridge (left), LASSO (right).
...
p_lasso <- tibble(
  ...
  geom_text(
    ...
    mapping = aes(x = x, y = y, label = label),
    parse = T, hjust = 0, nudge_x = 0.05, nudge_y = 0.1,
    color = col2
  ) +
  coord_equal(
    xlim = c(-1.1, 2.9),
...
```{r, plot-opt-ridge-lasso-print, out.width = "50%", fig.align='center', fig.cap = ifelse(knitr::is_html_output(), "(ref:opt-ridge-lasso-caption-gitbook)", "(ref:opt-ridge-lasso-caption-latex)"), fig.scap = "(ref:opt-ridge-lasso-caption-short)", label="opt-ridge-lasso", echo=FALSE, fig.ncol=2, fig.subcap=c('Ridge', 'LASSO'), cache=TRUE}
...
In this section we are going to present the Bayesian estimators for GLM.The novelty compared to Maximum Likelihood estimators consists in the fact that Bayesian statistics introduce the idea of prior information that introduce a bias in the estimates. As we will see in section \@ref(chap:ridge-lasso-bayes), Ridge Regression and LASSO Regression can be interpreted as bayesian estimators.


#### The Bayesian framework

In classical inference what is commonly done for estimating an unknown parameter $\theta$ using an observed sample $\boldsymbol{y}$, is to considerate the value $\hat{\theta}^{ML}$ that maximize the likelihood:
$$
\hat{\theta}^{ML} = \argmax_{\theta\in\Theta}{\ell(\theta)} = \argmax_{\theta\in\Theta}{p(\boldsymbol{y}|\theta)}
$$
where $p(\boldsymbol{y}|\theta)$ is the probability (or the density) of the sample $\boldsymbol{y}$ given the parameter $\theta$. That means that, within all the possible parameters $\theta\in\Theta$, we select the most likely one, that is the one that, conditioned to it, returns the maximum probability for the observed sample $\boldsymbol{y}$. We hilight that $\ell(\theta)$ is not a probability distribution on $\theta$, so for example, in general, $\int_{\Theta}{\ell(\theta)d\theta}\ne1$.

The _Bayesian Inference_ introduces the concept of prior distribution over $\theta$: $\pi(\theta)$. This distribution represents how probable we assume the different values of $\theta\in\Theta$ are on our mind prior to the observation of the sample. In classical statistics, the parameter $\theta$ is seen as a specific real number that is unknown. In the Bayesian framework, the parameter $\theta$ is seen as a random variable and its distribution represent our information on it. This aspect introduces a subjective point of view of probability.
...
$\pi(\theta|\boldsymbol{y})$ is actually a probability distribution, so we can compute probabilities and take prediction on $\theta$ given the distribution $\pi(\theta|\boldsymbol{y})$.

Two useful stastistics based on the posterior distribution $\pi(\theta|\boldsymbol{y})$ are the expected value of $\theta$ given $\boldsymbol{y}$:
$$
E(\theta|\boldsymbol{y}) = \int_{\Theta}{\theta \pi(\theta|\boldsymbol{y}) d\theta}
$$
and the mode of $\theta$ given $\boldsymbol{y}$:
$$
Mode(\theta|\boldsymbol{y}) = \argmax_{\theta\in\Theta}{\pi(\theta|\boldsymbol{y})}
$$

These two statistics can be used as point estimates for the parameter $\theta$. In the following we are going to refer to $Mode(\theta|\boldsymbol{y})$ as the _Maximum a Posteriori_ (MAP) estimate:
$$
\hat{\theta}^{MAP} = Mode(\theta|\boldsymbol{y}) = \argmax_{\theta\in\Theta}{\pi(\theta|\boldsymbol{y})}
$$
...
Substituting $p(\boldsymbol{y}|\mu)$ and $\pi(\mu)$ with the distributions assumed, we get:
$$
...
#### LASSO and Ridge Regression as Bayesian estimators {#chap:ridge-lasso-bayes}
...
#### Some considerations on Bayesian estimators
...
<!--
Tutti hanno algoritmi efficienti
Diverse soluzioni con diversi programmi
h2o
-->
