...
From \@ref(eq:exp-fam-expected-value) we we obtains that, as $\mu = b'(\theta)$, choosing the canonical function corresponds in using $\theta$ as the linear predictor:
$$
...
Therefore, fitting the model corresponds to estimate $\boldsymbol{\beta}, \phi$. The technique used in GLM to estimate $\boldsymbol{\beta}, \phi$ is the _maximum likelihood_.
