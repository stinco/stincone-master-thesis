...
If we assume that the explanatory variables are independent, i.e. $\boldsymbol{X}^t\boldsymbol{X} = I_p$ we can further simplify \@ref(eq:ridge-estimator) to:
$$
...
$$
and then $\text{tr}\left(H_{\lambda}\right) = \frac{p}{1+\lambda}$. This result means that, by increasing $\lambda$, the effective number of degrees of fredom decrease.
...
\hat{\boldsymbol{\beta}} = \argmin_{\boldsymbol{\beta}}{\left(D(\boldsymbol{\beta}, \boldsymbol{y}) + \lambda \|\boldsymbol{\beta}_{\setminus0}\|_1\right)}
\end{equation}
where:
...
(ref:opt-ridge-lasso-caption-latex) Optimization Ridge vs LASSO.
...
```{r, plot-opt-ridge-lasso-print, out.width = "50%", fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:opt-ridge-lasso-caption-gitbook)", "(ref:opt-ridge-lasso-caption-latex)"), label="opt-ridge-lasso", echo=FALSE, fig.ncol=2, fig.subcap=c('Ridge', 'LASSO'), cache = TRUE}
