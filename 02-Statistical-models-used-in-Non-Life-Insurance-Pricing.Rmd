...
(ref:gam-lambda-caption-latex) GAM estimate $\hat{f}(x)$ for different levels of the smoothing parameter $\lambda$.

(ref:gam-lambda-caption-gitbook) GAM estimate $\hat{f}(x)$ for different levels of the smoothing parameter $\lambda$, $\lambda = 0$ (top-left), $\lambda = 10$ (top-right), $\lambda = 10^3$ (bottom-left) and $\lambda = 10^6$ (bottom-right).
...
It can be proven that the trace of $H$ measures the flexibility of the function. Indeed, in Linear Model $L = \boldsymbol{X} \left( \boldsymbol{X}^t\boldsymbol{X} \right)^{-1}\boldsymbol{X}^t$ and $\text{tr}(H) = \text{tr}\left( \boldsymbol{X} \left( \boldsymbol{X}^t\boldsymbol{X} \right)^{-1}\boldsymbol{X}^t \right) = p + 1$, that is the number of _degrees of freedom_ of the model. For this reason $\text{tr}(H)$ is also called number of _effective degrees of freedom_ of the model. In GAM, as the smoothing parameter $\lambda$ increases, the flexibility of the model decreases and $\text{tr}(H)$ decreases.
...
One shrinkage estimator for GLM is the _Ridge_ estimator. In Ridge Regression, the decrease in variance of $\tilde{\boldsymbol{\beta}}$ is achieved by considering the optimization problem \@ref(eq:max-lik-est-deviance) and adding to the Deviance a penalization term that depends on the magnitude of $\beta_1, \beta_2, \dots, \beta_p$:
\begin{equation}
...
```{r, plot-ridge-lambda-print, out.width = c("50%", "50%", "50%", "50%", "70%"), fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:ridge-lambda-caption-gitbook)", "(ref:ridge-lambda-caption-latex)"), label="ridge-lambda", echo=FALSE, fig.ncol=2, fig.subcap = c(str_c('$\\lambda = ', lambda_ridge, '$'), '$\\hat{\\beta}_j$ for different values of $\\lambda$'), cache = TRUE}
...
If we assume that the explanatory variables are independent, i.e. $\boldsymbol{X}^t\boldsymbol{X} = I_p$ we can further simplify the expression \@ref(eq:ridge-estimator) to:
$$
...
```{r, plot-lasso-lambda-print, out.width = c("50%", "50%", "50%", "50%", "70%"), fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:lasso-lambda-caption-gitbook)", "(ref:lasso-lambda-caption-latex)"), label="lasso-lambda", echo=FALSE, fig.ncol=2, fig.subcap = c(str_c('$\\lambda = ', lambda_lasso, '$'), '$\\hat{\\beta}_j$ for different values of $\\lambda$'), cache = TRUE}
...
This property of the LASSO Regression can be derived from a different representation of the optimization problems \@ref(eq:ridge-est-deviance) and \@ref(eq:lasso-est-deviance). It can be proven that in general, considering a penalization given by the $L^p$ norm, the unconstrained optimization problem:
\begin{equation}
