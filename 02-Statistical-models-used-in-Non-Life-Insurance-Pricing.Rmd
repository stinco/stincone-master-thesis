...
The term $\|\boldsymbol{\beta}_{\setminus0}\|_2^2$ produces a penalization for high values of $\beta_j$ that leads to a shrinkage of the coefficients.
...
As for GAM, if $\lambda=0$, the optimization problem \@ref(eq:ridge-est-deviance) corresponds to the maximum likelihood, while for $\lambda\to+\infty$ the optimal point is $\hat{\boldsymbol{\beta}} = \left(\bar{y}, 0, \dots, 0\right)$, that corresponds to the trivial model with no explanatory variables. An example of the effect of $\lambda$ is reported in figure \@ref(...). The optimal point for $\lambda=0$ can be obtained through a Cross Validation as seen in section \@ref(chap:gam-choice-lambda).
...
An example of the effect of the $L^1$ penalization for different values of $\lambda$ is shown in figure \@ref(...).
