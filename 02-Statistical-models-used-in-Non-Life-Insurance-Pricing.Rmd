...
The higher $K$ is, the less subjected to randomness $D^{CV(K)}$ is. However, the higher $K$ is, the computationally heavier the procedure is. A common choice for $K$ is $K=10$. If we choose $K=n$, the procedure is also called _leave-one-out cross validation_.
...
{\left(
  D(\boldsymbol{f}, \boldsymbol{y})
    + \sum_{l=1}^{q}{
      \lambda_l \int_{a_l}^{b_l}{\left( f_l''(x) \right)^2 dx}
    }
\right)} 
\end{equation}
...
As described, the selection of the $x_l$ effect consists just in finding the optimal parameter $\lambda_l$. The technique commonly used in machine learning for hyperparameter tuning is the _Cross Validation_.

For a set $\Lambda$ of values of $\boldsymbol{\lambda}$ we can perform a K-fold cross validation as described in \@ref(chap:variable-selection) and, for each vector $\boldsymbol{\lambda}\in\Lambda$, compute the average test deviance:
$$
...
This formula is particularly convenient because it requires just the fitting with the whole dataset and not one fit for each fold. Indeed, in the formula the quantities that must be computed are $\mu_i$ and $L_{ii}$, that both depend on the model fitted with the whole dataset.
...
In section \@ref(chap:cubic-splines) we said that natural cubic splines on knots $\nu_1, \dots, \nu_m$ contitute a $m$-dimensional vector space and that a possible basis decomposition is \@ref(eq:cubic-spline-decomposition).
...
For $j \ge 0$ and $k\in\{1,2,\dots,m+j-1\}$:
$$
B_{j,k}(x) = 
\frac{x-\nu_{k-j-1}}{\nu_{k}-\nu_{k-j-1}} B_{j,k-1}(x)
+ \frac{\nu_{k+1}-x}{\nu_{k+1}-\nu_{k-j}} B_{j,k}(x)
...
```{r, plot-b-splines-print, out.width = "35%", fig.align='center', fig.cap = ifelse(knitr::is_html_output(), "(ref:plot-b-splines-caption-gitbook)", "(ref:plot-b-splines-caption-latex)"), label = "b-splines-plot", echo = FALSE, fig.ncol = 3, fig.subcap = c('degree $1$', 'degree $2$', 'degree $3$'), cache = TRUE}

plot_grid_split(p_b_splines_1, p_b_splines_2, p_b_splines_3)

```

It can be proven that \@ref(def:b-splines) defines functions $B_{j,k}(x)$ that, given $j$, are splines of degree $j$. Moreover, $B_{j,1}, \dots, B_{j,m+j-1}$ constitute a basis of the vector space of the splines of degree $j$ on the knots $\nu_1, \dots, \nu_m$. In particular, $B_{3,1}, \dots, B_{j,m+2}$ consitute a basis of the vector space of the splines of degree $3$. Therefore, if $s(x)$ is third degree spline on the knots $\nu_1, \dots, \nu_m$, it can be expressed as a linear combination of $B_{3,1}, \dots, B_{3,m+2}$:
$$
