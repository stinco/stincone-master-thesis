...
Figure \@ref(fig:bayes-normal) shows the prior $\pi(\mu)$, the likelihood $\pi(\mu|\boldsymbol{y})$ and the posterior distribution $\pi(\mu|\boldsymbol{y})$ for different values of the initial parameters and for different sample sizes $n$. As we can see, $\pi(\mu|\boldsymbol{y})$ lie between $\pi(\mu)$ and $\pi(\mu|\boldsymbol{y})$.
...
The Bayesian approach can be applied to GLM by adopting prior distribution on the coefficients $\boldsymbol{\beta}, \sigma^2$.
