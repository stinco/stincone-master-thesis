...
* $\boldsymbol{x}_i$ is the vector of the variables with a linear effect as described in GLM, that also include a term $1$ that represent the intercept;
...
* $f_1(\cdot), f_2(\cdot), \dots, f_q(\cdot)$ are continous functions with first and second derivative continuous.
...
Given $m$ real numbers $\nu_1, \nu_2, \dots, \nu_m$, called \textit{knots}, and given a function $f:[\nu_1, \nu_m] \to \mathbb{R}$ such that:
$$
f(x) = h_t(x), \qquad x\in[\nu_t, \nu_{t+1}[
$$
where, for $t = 1, 2, \dots, m-1$, $h_t(x) = \alpha_t + \vartheta_t x + \gamma_t x^2 + \delta_t x^3$ and, for the last index $m-1$, $f(x) = h_t(x)$ is extended to $[\nu_t, \nu_{t+1}]$

$f(x)$ is called \textit{cubic spline} if it satisfies the following conditions in the internal knots $\nu_2, \nu_3, \dots, \nu_{m-1}$
\begin{equation}
\label{eq:cubic-spline-contraints}
h_{t-1}(\nu_t) = h_{t}(\nu_t), \qquad h'_{t-1}(\nu_t) = h'_{t}(\nu_t), \qquad h''_{t-1}(\nu_t) = h''_{t}(\nu_t)
\end{equation}
```

The constraints for $f(\cdot)$ make it a continous functions with first and second derivative continuous in $]\nu_1, \nu_m[$. It is possible to extend the cubic spline $f(\cdot)$ to an interval $[a, b] \supset [\nu_1, \nu_m]$ with linear extensions on $[a, \nu_1[$ and $]\nu_m, b]$. The function $f:[a,b]\to\mathbb{R}$ is called _natural cubic spline_.

In definition \@ref(def:cubic-splines) we introduced 4 parameters ($\alpha_t, \vartheta_t, \gamma_t, \delta_t$) for each of the $m-1$ inteevals $[\nu_t, \nu_{t+1}[$, $t\in\{1,2,\dots,m-1\}$. So, we have $4(m-1)$ parameters. In equation \@ref(eq:cubic-spline-contraints) we introduced $3$ constraints for each of the $m-2$ knot in $\nu_2, \nu_3, \dots, \nu_{m-1}$. So the free parameters becomes $4(m-1) - 3(m-2) = m+2$. The linear extension on $[a, \nu_1[$ and $]\nu_m, b]$ corresponds to the constraint $f''(x)=0$ on $[a, \nu_1[ \, \cup \, ]\nu_m, b]$, thus, $h''_1(\nu_1)=0$ and $h''_m(\nu_m)$. Adding these two constraints, we get that the natural cubic spline $f:[a,b]\to\mathbb{R}$ has $m$ degrees of freedom.
...
```{r, plot-cub-spline-build, echo = FALSE}
...
```{r, plot-cub-spline-print, out.width = "50%", fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:cub-spline-caption-gitbook)", "(ref:cub-spline-caption-latex)"), label="cub-spline", echo=FALSE, fig.ncol=2, fig.subcap=c('$0$ knots', '$3$ knots', '$7$ knots', '$35$ knots')}
...
A measure of the wigglyness of a function is given by the integral of its squared second derivative:
\begin{equation}
\label{eq:wigglyness}
...
```{r, plot-wiggliness-print, out.width = "100%", fig.width = 10, fig.height = 5, fig.align='center', fig.cap="(ref:wiggliness-caption-latex)", label="wiggliness", echo=FALSE}
...
As we can see from the plots, if a function $f(x)$ is linear, $\left(f''(x)\right)^2$ is null and, as the curvature of the function increases, $\left(f''(x)\right)^2$ increses. For this reason, $\left(f''(x)\right)^2$ can also be seen as a measure of how much $f(x)$ differs from a linear function.
...
<!-- \begin{equation} -->
<!-- \label{eq:gam-est-deviance} -->
<!-- \hat{\boldsymbol{f}} = \argmin_{\boldsymbol{f}}{\left(D(\boldsymbol{f}, \boldsymbol{y}) + \sum_{l=1}^{s}{\lambda_l \int_{a_l}^{b_l}{\left( f''_l(z_l) \right)^2 dz_l}}\right)} -->
<!-- \end{equation} -->

In the notation of the formula \@ref(eq:gam-est-deviance) we used $f$ to indicate all the parameters of the natural cubic spline. The hyper-parameter $\lambda>0$ is called _smoothing parameter_. It measures how much we want to penalize wiggliness. If we choose $\lambda=0$ we won't penalize for $\left( f''(x) \right)^2$ and the optimization problem correspond to the maximum likelihood one. The higher $\lambda$ is, the more penalization for $\left( f''(x) \right)^2$ we introduce and the more regual the estimate $\hat{f}(x)$ will be. If $\lambda\to+\infty$, we will introduce an infinite penalization for wiggliness, so the result will have $\hat{f}''(x)=0$, thus $\hat{f}(x)$ will be linear. An example with different levels of $\lambda$ can be seen in figure \@ref(fig:gam-lambda).
...
```{r, plot-gam-lambda-build, echo = FALSE}
...
```{r, plot-gam-lambda-print, out.width = "50%", fig.align='center', fig.cap=ifelse(knitr::is_html_output(), "(ref:gam-lambda-caption-gitbook)", "(ref:gam-lambda-caption-latex)"), label="gam-lambda", echo=FALSE, fig.ncol=2, fig.subcap=c('$\\lambda = 0$', '$\\lambda = 10$', '$\\lambda = 10^3$', '$\\lambda = 10^6$')}
...
As soon as the number of knots $m$ is large enough, the exact number $m$ and the positioning of the knots $\nu_1, \nu_2, \dots, \nu_m$ is not important. If the function is flexible enough, the tuning of the curve is done by just tuning $\lambda$. It is possible to use as many knots as the observations are, but it could be too computationally heavy. A possible choice is to fix a number of knots $m$ and positioning $\nu_1, \nu_2, \dots, \nu_m$ on empirical quantiles of the observed $x$ or equally spaced in the range of $x$. In our example, $m=50$ knots seems to be large enough.

Now the selection of the $x$ effect consists just in finding the optimal parameter $\lambda$. To do so, for a set $\Lambda$ of values of $\lambda$ we can perform a K-fold cross validation as described in \@ref(chap:var-effects) and, for each $\lambda\in\Lambda$, compute the average test deviance:
$$
D^{CV}_\lambda = \frac{1}{K} \sum_{k=1}^{K}{D\left( \hat{f}_\lambda^{\setminus \mathcal{B}_k}, \boldsymbol{y}^{\mathcal{B}_k} \right)}
$$
At this point we will choose the hyperparameter that minimizes the cross-validation deviance:
$$
\hat{\lambda} = \argmin_{\lambda\in\Lambda}{D^{CV}_\lambda}
$$
