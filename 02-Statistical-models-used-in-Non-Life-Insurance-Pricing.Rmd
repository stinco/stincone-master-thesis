...
\hat{\boldsymbol{\beta}} = \argmin_{\boldsymbol{\beta}\in\mathbb{R}^{p+1}}{D(\hat{\boldsymbol{\beta}}, \boldsymbol{y})}
\end{equation}
...
Given $m$ real numbers $\nu_1, \nu_2, \dots, \nu_m$, called \textit{nodes}, and given a function $f:[\nu_1, \nu_m] \to \mathbb{R}$ such that:
$$
...
If we try to fit a cubic spline on data, we find that by increasing the number of knots, the function tends to overfit data. This is due to the fact that, increasing the number of knots, we are increasing the number of parameters of the model and thus the variance of the estimators for the parameters increase. We can see this effect in figure \@ref(fig:cub-spline).
...
p_cub_spline_4 <- plot_spline_lidar(dataset = lidar,
                        nodes = seq(from = -1.7, to = 1.7, by = .1),
                        p = 3)
```
...
A measure of the wigglyness of a function is given by the integral of its second derivative squared:
\begin{equation}
\label{eq:wigglyness}
...
What is done in GAM to contain this wiggliness and to prevent overfitting is to take into account a penalization for wiggliness. This can be achieved by considering the minimization problem in \@ref(eq:max-lik-est-deviance) and adding a penalization term to the deviance:
\begin{equation}
\label{eq:gam-est-deviance}
\hat{\boldsymbol{\beta}} = \argmin_{\boldsymbol{\beta}\in\mathbb{R}^{p+1}}{\left(D(\hat{\boldsymbol{\beta}}, \boldsymbol{y}) + \sum_{l=1}^{s}{\lambda_l \int_{a_l}^{b_l}{\left( f''(z_l) \right)^2 dz_l}}\right)}
\end{equation}
